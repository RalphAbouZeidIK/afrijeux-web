...
export class CartComponent implements OnInit {
  ...
  constructor(
    ...
    this.isSportsBetting = location.pathname.split('/')[1] == 'Sports'
    ...
    this.isPMU = location.pathname.split('/')[1] == 'PMU'
  ...
  ngOnInit(): void {
    ...
    if (this.isSportsBetting) {
      this.cartInitialize(this.storageSrv.getItem('cartData'))
    }
  ...
  clearBets() {
    this.storageSrv.removeItem('totalBets')
  ...
  onAddQtty(betItem: any) {
    if (betItem.multiplicator == betItem.selectedFixedConfig.MaxMultiplier) {
      return
    }
    this.cartSrv.updateMultiplicator(betItem, 1)
  }

  onRemoveQtty(betItem: any) {
    if (betItem.multiplicator == 1) {
      return
    }
    this.cartSrv.updateMultiplicator(betItem, -1)
  }
